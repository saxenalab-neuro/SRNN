...
def get_gamma(forward_prob,backward_prob,prob_all_s,prob_all_h,prob_all_y):
    # see https://en.wikipedia.org/wiki/Baum%E2%80%93Welch_algorithm for the theory of baum welch
    # gamma1 is log p(z_t=i|y,theta), note we use log likelihood here
    ...
    # delta1 is log p(z_t=i,z_t+1=j|y,theta)
    delta1=torch.zeros((forward_prob.shape[0],forward_prob.shape[1]-1,forward_prob.shape[2],forward_prob.shape[2]))
    for t in range(forward_prob.shape[1]-1):
        for i in range(forward_prob.shape[2]):
            for j in range(forward_prob.shape[2]):
                delta1[:,t,i,j]=forward_prob[:,t,i]+prob_all_s[:,t,i,j]+backward_prob[:,t+1,j]+prob_all_h[:,t+1,j]+prob_all_y[:,t+1]
    sum_delta1=torch.zeros((forward_prob.shape[0],forward_prob.shape[1]-1))
    for t in range(forward_prob.shape[1]-1):
        sum_delta1_each=torch.zeros((forward_prob.shape[0],forward_prob.shape[2],forward_prob.shape[2]))
        for k in range(forward_prob.shape[2]):
            for w in range(forward_prob.shape[2]):
                sum_delta1_each[:,k,w]=forward_prob[:,t,k]+prob_all_s[:,t,k,w]+backward_prob[:,t+1,w]+prob_all_h[:,t+1,w]+prob_all_y[:,t+1]
        sum_delta1[:,t]=torch.logsumexp(sum_delta1_each.reshape(forward_prob.shape[0],forward_prob.shape[2]*forward_prob.shape[2]),1)
