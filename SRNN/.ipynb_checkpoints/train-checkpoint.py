...
def train_(model,rnninfer,optimizer,scheduler,X_train,y_train,X_test,y_test,beh_labels,beh_labels_test,num_tv,epochs,device):
    ...
    for epoch in range(epochs):
        ...
        prob_ini,prob_all_s,prob_all_h,prob_all_y,gamma1,gamma2,fwp,bwp = model(X_train,y_train,sampled_h,device)
        t1,t2=loss_function.get_loss(gamma1,gamma2,prob_ini.cpu(),prob_all_s,prob_all_h,prob_all_y)
...
def eval_(model,rnninfer,X_test,y_test,device):
    ...
    prob_ini_test,prob_all_s_test,prob_all_h_test,prob_all_y_test,gamma1_test,gamma2_test,fwp_test,bwp_test = model(X_test,y_test,sampled_h_test,device)
    
    t1_test,t2_test=loss_function.get_loss(gamma1_test,gamma2_test,prob_ini_test.cpu(),prob_all_s_test,prob_all_h_test,prob_all_y_test)
