...
class RNNInfer(nn.Module):
    def __init__(self,input_shape,hidden_shape):
        ...
        self.forward_rnn = nn.RNN(input_size=self.input_shape, hidden_size=self.hidden_shape, num_layers=1, batch_first=True)
        self.backward_rnn = nn.RNN(input_size=self.input_shape, hidden_size=self.hidden_shape, num_layers=1, batch_first=True)
        self.rnn_mean=nn.RNN(input_size=self.hidden_shape, hidden_size=self.hidden_shape, num_layers=1, batch_first=True)
    ...
    def forward(self, y_train):
        ...
        f_out,_=self.forward_rnn(y_train,h_f0)
        b_out,_=self.backward_rnn(torch.flip(y_train,dims=(1,)),h_b0)
        bi_output=f_out+b_out
        ...
        for i in range(y_train.shape[1]):
            ...
            sampled_h_=mean_out_+ep[:,None,:]*covariance_matrix.diag()
            sampled_h[:,i,:]=sampled_h_[:,0,:]
            mean_out[:,i,:]=mean_out_[:,0,:]
        infer_dist=torch.distributions.multivariate_normal.MultivariateNormal(mean_out[0], covariance_matrix)
